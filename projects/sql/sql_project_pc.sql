...
SELECT DISTINCT f.name as facilityName, CONCAT(m.firstname,' ', m.surname) as memberName 
FROM Bookings b INNER JOIN Facilities f ON b.facid = f.facid
...
SELECT DISTINCT f.name as facilityName, CONCAT(m.firstname,' ', m.surname) as memberName, b.slots,
CASE 
...
ORDER BY (CASE WHEN m.memid =0 THEN f.guestcost ELSE f.membercost END) DESC
